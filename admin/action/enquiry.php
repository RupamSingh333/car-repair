...
switch ($action) {
    case "save":
        ...
        $output =  save_command(tbl_enquiry, $field, "user_id", $primary_value);
        $_SESSION['msg'] = $output;
        break;
    ...
    case "del":
        ...
        $output =  del_command(tbl_enquiry, "user_id", $primary_value, false);
        $_SESSION['msg'] = $output;
        break;
    case "status":
        if (isset($_GET['id'])) {
            $id = urlencode(decryptIt($_GET['id']));
            $row = getEnquiry_byID($id);
            $st = $row['status'];
        }
        if ($st == "0") {
            $status = "1";
